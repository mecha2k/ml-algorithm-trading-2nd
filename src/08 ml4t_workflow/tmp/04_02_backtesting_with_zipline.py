...
# improves efficiency because it optimizes computations over the entire backtest period rather than tackling each
# event separately. In other words, it continues to follow an event-driven architecture but vectorizes the computation
# of factors where possible.
## A Pipeline uses Factors, Filters, and Classifiers classes to define computations that produce columns in a table
# with PIT values for a set of securities. Factors take one or more input arrays of historical bar data and produce
# one or more outputs for each security. There are numerous built-in factors, and you can also design your own
# `CustomFactor` computations.
...
# You need to register your Pipeline with the `initialize()` method and can then execute it at each time step or
# on a custom schedule. Zipline provides numerous built-in computations such as moving averages or Bollinger Bands
# that can be used to quickly compute standard factors, but it also allows for the creation of custom factors as
...
def load_predictions(bundle):
    predictions = pd.read_hdf("../data/backtest_08.h5", "data")[["predicted"]].dropna()
    ...
    # `zipline.pipeline.DataSset` and contains a single `zipline.pipeline.Column` of type `float` and
    # has the domain `US_EQUITIES`:
...
# short positions. To get there, we first define a `CustomFactor` called `MLSignal` that just receives the current
...
### Create Pipeline
# Now we create a `compute_signals()` that returns a `zipline.pipeline.Pipeline` which filters the assets
# that meet our long/short criteria. We will call ths function periodically while executing the backtest.
# More specifically, we set up our Pipeline by instantiating the `CustomFactor` that requires no arguments
# other than the defaults. We combine its `top()` and `bottom()` methods with a filter to select the highest
# positive and lowest negative predictions:
def compute_signals():
    signals = MLSignal()
...
if __name__ == "__main__":
    ...
    # Load the Wiki Quandl `bundle` data that we ingested earlier using `zipline ingest`. This gives us access to
    # the security SID values, among other things.
    ...
    # In fact, we need to slightly modify the Zipline library’s source code to bypass the assumption that we will
    # only load price data. To this end, we will add a `custom_loader` parameter to the `run_algorithm` and ensure
    # that this loader is used when the `Pipeline` needs one of `SignalData`’s `Column` instances.
    ...
    fig.tight_layout()
    plt.savefig("images/04_02_01.png", dpi=300, bboxinches="tight")
    ...
    fig.savefig("images/04_02_02.png", dpi=300)
