...
# we used in [backtesting_with_zipline](02_backtesting_with_zipline.ipynb) and generated in [Chapter 7]
# (../../07_linear_models). We will, however, use a few additional Pipeline factors to illustrate their usage.
# The principal new element is a CustomFactor that receives features and returns as inputs to train a model and produce
# predictions. We follow the workflow displayed in the following figure:
...
### Price Trend
# A linear price trend that we estimate using linear regression (see [Chapter 7](../../07_linear_models) works as
# follows: we use the 252 latest close prices to compute the regression coefficient on a linear time trend:
...
# The `evaluate_predictions()` function does exactly this: it tracks the past predictions of our model and evaluates
# them once returns for the relevant time horizon materialize (in our example the next day):
...
if __name__ == "__main__":
    ...
    # volatility. Next, we’ll come up with a CustomFactor that trains our model.
    ...
    plt.savefig("images/04_03_01.png", dpi=300, bboxinches="tight")
    ...
    fig.tight_layout()
    plt.savefig("images/04_03_02.png", dpi=300, bboxinches="tight")
    ...
    fig.savefig("images/04_03_03.png", dpi=300, bboxinches="tight")
