...
def get_equities():
    ...
    nasdaq_sids = equities.loc[equities.symbol.isin(nasdaq_tickers.symbol), ["symbol", "sid"]]
    nasdaq_tickers = (
        nasdaq_tickers.merge(nasdaq_sids, how="left")
        .reset_index()
...
def get_dividends():
    ...
    dividends = (
        dividends.rename(columns={"sid": "quandl_sid"})
...
def get_splits():
    ...
    splits = (
        splits.rename(columns={"sid": "quandl_sid"})
        .merge(equities[["quandl_sid", "sid"]])
        .drop("quandl_sid", axis=1)
    )
...
def get_ohlcv_by_ticker():
    ...
    nasdaq100 = (
        pd.read_hdf(data_path / "data.h5", "1min_trades")
    ...
    for symbol, data in nasdaq100.groupby(level="ticker"):
        ...
        data.reset_index("ticker", drop=True).to_hdf(
            "algoseek.h5", "{}".format(symbol_dict[symbol])
        )
