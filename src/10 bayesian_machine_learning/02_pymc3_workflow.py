...
if __name__ == "__main__":
    ...
    # data.to_pickle("../data/ind_fred.pkl")

    data = pd.read_pickle("../data/ind_fred.pkl").ffill().resample("M").last().dropna()
    ...
    graph.save("../data/10_log_reg.dot")
    ...
    def plot_traces(traces, burnin=2000):
        ...
        ax = pm.plot_trace(traces[burnin:], figsize=(15, len(traces.varnames) * 1.5), lines=summary)
    ...
    with logistic_model:
        trace_mh = pm.sample(
            tune=1000, draws=5000, step=pm.Metropolis(), cores=4, return_inferencedata=True
        )
