...
# (https://www.algoseek.com/) (available [here](https://www.algoseek.com/ml4t-book-data.html)) and engineer a few
# illustrative features.
# The rich set of trade and quote information contained in the Algoseek data offers various opportunities to add
# inforation, e.g. about relative spreads and demand/supply imbalances, but since the data is fairly large we limit
# our efforts to a small number of features.
# > Note that we will assume throughout that we can always buy (sell) at the first (last) trade price for a given bar
# at no cost and without market impact; this is unlikely to be true in reality but simplifies the example).
...
# The Quote fields are based on changes to the NBBO ([National Best Bid Offer]
# (https://www.investopedia.com/terms/n/nbbo.asp)) from the top-of-book price and size from each of the exchanges.
...
# The field `Volume` measuring total number of shares traded in bar will be `0` if there are no Trades (see `No Value`
...
# To provide consistent Bid/Ask prices at a point in time while showing the low/high Bid/Ask for the bar, AlgoSeek uses
# the low/high `Bid` and the corresponding `Ask` at that price.
#
# ### FAQ
...
# Very often a Trade may not occur at these prices as the price may only last a few seconds or executions are being
# crossed at mid-point due to hidden order types that execute at mid-point or as price improvement over current `Bid`/`Ask`.
#
...
#
# When a trade is done that is off the listed exchanges, it must be reported to FINRA by the brokerage firm or dark
# pool. Examples include:
# - internal crosses by broker dealer
# - over-the-counter block trades, and
# - dark pool executions.

## Data prep
# We use the 'Trade and Quote' dataset - see [documentation]
# (https://www.algoseek.com/old_static/files/documentation/equity_and_etf_etn/AlgoSeek.US.Equity.TradeAndQuotes.OHLC.Bars.1.5.pdf)
# for details on the definition of the numerous fields.
...
if __name__ == "__main__":
    ...
    # The Algoseek minute-bar data comes in compressed csv files that contain the data for one symbol and day,
    # organized in three directories for each year (2015-17). The function `extract_and_combine_data` reads the ~80K
    # source files and combines them into a single `hdf5` file for faster access.
    # >The data is fairly large (>8GB), and if you run into memory constraints, please modify the code to process the
    # data in smaller chunks. One options is to iterate over the three directories containing data for a single year
    # only, and storing each year separately.
    ...
    for feature in tqdm(features):
        ...
        by_day = df.groupby(
            df.index.get_level_values("date_time").date
