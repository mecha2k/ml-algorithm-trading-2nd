...
# In this notebook, we load the high-quality NASDAQ100 minute-bar trade-and-quote data generously provided by
# [Algoseek](https://www.algoseek.com/) (available [here](https://www.algoseek.com/ml4t-book-data.html)) that we will
# use in [Chapter 12](../../12_gradient_boosting_machines) to develop an intraday trading strategy.
...
# (https://www.investopedia.com/terms/n/nbbo.asp)) from the top-of-book price and size from
...
# crossed at mid-point due to hidden order types that execute at mid-point or as price improvement over
# current `Bid`/`Ask`.
#
...
# (https://us-equity-market-data-docs.s3.amazonaws.com/algoseek.US.Equity.TAQ.Minute.Bars.pdf) for details on the
# definition of the numerous fields.
...
# The Algoseek minute-bar data comes in compressed csv files that contain the data for one symbol and day, organized in
# three directories for each year (2015-17). The function `extract_and_combine_data` reads the ~80K source files and
# combines them into a single `hdf5` file for faster access.
# >The data is fairly large (>8GB), and if you run into memory constraints, please modify the code to process the data
# in smaller chunks. One options is to iterate over the three directories containing data for a single year only, and
# storing each year separately.
...
print(len(df.index.unique("ticker")))

# The below heatmap highlights the frequent entry/exit points of various securities, which emphasizes the need for
# a survivorship-free dataset.
constituents = (
