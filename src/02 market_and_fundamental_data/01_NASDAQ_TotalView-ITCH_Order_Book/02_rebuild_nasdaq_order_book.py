...
# historical data for free. The trading activity is reflected in numerous messages about trade orders sent by market
# participants. These messages typically conform to the electronic Financial Information eXchange (FIX) communications
# protocol for real-time exchange of securities transactions and market data or a native exchange protocol.
...
### Set Data paths
# We will store the download in a `data` subdirectory and convert the result to `hdf` format (discussed in the last
...
# The parsed messages allow us to rebuild the order flow for the given day. The 'R' message type contains a listing of
# all stocks traded during a given day, including information about initial public offerings (IPOs) and trading
# restrictions. Throughout the day, new orders are added, and orders that are executed and canceled are removed from
# the order book. The proper accounting for messages that reference orders placed on a prior date would require tracking the order book over multiple days, but we are ignoring this aspect here.

### Get all messages for given stock
# The `get_messages()` function illustrates how to collect the orders for a single stock that affects trading (refer
# to the ITCH specification for details about each message):
...
# Reconstructing successful trades, that is, orders that are executed as opposed to those that were canceled from
# trade-related message types, C, E, P, and Q, is relatively straightforward:
def get_trades(m):
...
### Create Orders
# The order book keeps track of limit orders, and the various price levels for buy and sell orders constitute the depth
# of the order book. To reconstruct the order book for a given level of depth requires the following steps:
# The `add_orders()` function accumulates sell orders in ascending, and buy orders in descending order for a given
# timestamp up to the desired level of depth:
...
# The number of orders at different price levels, highlighted in the following screenshot using different intensities
# for buy and sell orders, visualizes the depth of liquidity at any given point in time.
...
buy_per_min.timestamp = buy_per_min.timestamp.add(utc_offset).view(int)
buy_per_min.info()
...
sell_per_min.timestamp = sell_per_min.timestamp.add(utc_offset).view(int)
sell_per_min.info()
...
trades_per_min.index = trades_per_min.index.to_series().add(utc_offset).view(int)
trades_per_min.info()

# The following plots the evolution of limit orders and prices throughout the trading day: the dark line tracks the
# prices for executed trades during market hours, whereas the red and blue dots indicate individual limit orders on
# a per-minute basis (see notebook for details)
sns.set_style("white")
