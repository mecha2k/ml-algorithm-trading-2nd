...
# the text files to binary, columnar parquet format (see Section 'Efficient data storage with pandas' in chapter 2
# for a performance comparison of various data-storage options compatible with pandas DataFrames):
...
## Data Organization
# For each quarter, the FSN data is organized into eight file sets that contain information about submissions, numbers,
# taxonomy tags, presentation, and more. Each dataset consists of rows and fields and is provided as a tab-delimited
# text file:
...
# We can select a useful field, such as Earnings per Diluted Share (EPS), that we can combine with market data to
# calculate the popular Price/Earnings (P/E) valuation ratio.
stock_split = 7
split_date = pd.to_datetime("20140604")
print(split_date)


# We do need to take into account, however, that Apple split its stock 7:1 on June 4, 2014, and Adjusted Earnings per
# Share before the split to make earnings comparable, as illustrated in the following code block:
...
# We can select values of interest and track their value or use them as inputs to compute fundamental metrics like
# the Dividend/Share ratio.
