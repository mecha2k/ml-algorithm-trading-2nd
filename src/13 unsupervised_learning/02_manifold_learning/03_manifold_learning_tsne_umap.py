...
colors = ["denim blue", "pale red", "green"]
...
data, label = make_blobs(
    n_samples=300, n_features=2, cluster_std=[10, 1], centers=2, random_state=42
)
...
for row, perplexity in enumerate([2, 5, 10, 20, 30, 50, 100]):
    for col, n_iter in enumerate([250, 500, 1000, 3000, 5000]):
        ...
        axes[row, col].set_title(f"Perplexity: {perplexity} | Iter: {n_iter}", fontsize=18)
...
for row, neig in enumerate([2, 5, 10, 25, 50]):
    for col, minDist in enumerate([1, 10, 20, 50]):
        x, y = np.load(swiss_path / method / f"{neig}_{minDist}.npy").T
        axes[row, col].scatter(x, y, c=palette, s=5)
        axes[row, col].set_title(f"neig: {neig} | Min.Dist {minDist/100:.2f}", fontsize=18)
        axes[row, col].axis("off")
...
def plotly_scatter(data, label, title, color, x="x", y="y"):
    ...
    pyo.iplot(fig, show_link=True, filename="03-14", image="png")
...
# trace1 = go.Scatter(x=data[:, 0], y=data[:, 1], mode="markers", name="markers")
# layout = go.Layout(title="MNIST TSNE Projection")
