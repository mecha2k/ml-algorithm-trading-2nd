...
class StatisticalArbitrageCointegration(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if order_type in ["open", "close"]:
                ...
                if pair is None:
                    self.log(
                        f"{pair_id},{symbol},{order.ref},{order_type},Completed (missing),,{p},{s},{p * s}"
                    )
                    return
                ...
                self.log(
                    f"{pair_id},{symbol},{order.ref},{order_type},Completed,{pair.long},{p},{s},{p * s}"
                )
            else:
    ...
    def next(self):
        ...
        exposure = (
            pd.DataFrame({"price": pd.Series(self.last_close), "position": pd.Series(exp)})
            .replace(0, np.nan)
            .dropna()
        )
        ...
        if self.cnt % 21 == 0:
            holdings = pf - cash
            msg = f"PF: {pf:11,.0f} | Net: {holdings: 11,.0f} "
            msg += f"| # Pos: {len(exposure):3,.0f} | # Pairs: {len(self.active_pairs):3,.0f} | "
...
### Load Prices
prices = (
    pd.read_hdf(STORE, "prices").sort_index().loc[idx[traded_symbols, str(start) : str(end)], :]
)
...
cerebro.addstrategy(
    StatisticalArbitrageCointegration, trades=trades, verbose=True, log_file="bt_log.csv"
)
...
# The PyFolio integration is somewhat broken due to API changes after version 0.5.1, so we need to retrieve the
# positions manually.
