...
if __name__ == "__main__":
    ...
    # While exponential smoothing models are based on a description of the trend and seasonality in the data, ARIMA models
    # aim to describe the autocorrelations in the data. ARIMA(p, d, q) models require stationarity and leverage two
    # building blocks:
    ...
    print(model1.params.sort_index() == model2.params.sort_index().values)

    ### Seasonal differencing vs SARIMAX
    # Seasonal differencing has same effected as using SARIMAX w seasonal order (0,1,0,12).
    model1 = tsa.statespace.SARIMAX(
        industrial_production_log, order=(2, 0, 2), seasonal_order=(0, 1, 0, 12)
    ).fit()
    model2 = tsa.statespace.SARIMAX(
        industrial_production_log_diff, order=(2, 0, 2), seasonal_order=(0, 0, 0, 12)
    ).fit()
    ...
    for p in range(5):
        for q in range(5):
            ...
            result = (
                pd.DataFrame({"y_true": y_true, "y_pred": y_pred}).replace(np.inf, np.nan).dropna()
            )
    ...
    sns.heatmap(
        arma_results[arma_results.RMSE < 0.5].RMSE.unstack().mul(10),
        fmt=".3f",
        annot=True,
        cmap="Blues",
        ax=axes[0],
    ...
    sns.heatmap(
        arma_results.BIC.unstack(), fmt=".2f", annot=True, cmap="Blues", ax=axes[1], cbar=False
    )
    ...
    sarimax_model = tsa.SARIMAX(
        endog=industrial_production_log_diff.dropna().values,
        order=(2, 0, 2),
    ...
    # seasonal (lag-12) differences. We estimate the model for a range of both ordinary and conventional AR and MA
    # parameters using a rolling window of 10 years of training data, and evaluate the RMSE of the 1-step-ahead forecast.
    ...
    print(sarimax_results.nsmallest(5, columns="RMSE"))

    # We also collect the AIC and BIC criteria that show a very high rank correlation coefficient of 0.94, with BIC
    # favoring models with slightly fewer parameters than AIC. The best five models by RMSE are:
    print(sarimax_results[["RMSE", "AIC", "BIC"]].sort_values("RMSE").head())
    print(sarimax_results[["RMSE", "AIC", "BIC"]].corr("spearman"))
    ...
    print(
        sarimax_results[
            (sarimax_results.RMSE < sarimax_results.RMSE.quantile(0.05))
            & (sarimax_results.BIC < sarimax_results.BIC.quantile(0.1))
        ].sort_values("RMSE")
    )
    ...
    best_model = tsa.SARIMAX(
        endog=industrial_production_log_diff.values,
        order=(p1, 0, q1),
        seasonal_order=(p2, 0, q2, 12),
    ).fit()
