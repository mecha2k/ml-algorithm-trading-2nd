...
# The data consists of several files with information on the business, the user, the review and other aspects that Yelp
# provides to encourage data science innovation. We will use around six million reviews produced over the 2010-2019
# period to extract text features. In addition, we will use other information submitted with the review about the user.

# You can download the data from [here](https://www.yelp.com/dataset) in json format after accepting the license.
# The 2020 version has 4.7GB (compressed) and around 10.5GB (uncompressed) of text data.
...
def merge_files(remove=False):
    ...
    if not combined_file.exists():
        ...
        combined = review.merge(user, on="user_id", how="left", suffixes=["", "_user"]).drop(
            "user_id", axis=1
        )
