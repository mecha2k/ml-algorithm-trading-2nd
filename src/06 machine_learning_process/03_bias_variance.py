...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        y_["Train"] += normal(loc=0, scale=np.std(y_["Train"]) * noise, size=sample)
        ...
        for fit, model in trained_models.items():
            for dataset in datasets:
                ...
                result.append(
                    pd.DataFrame(
                        dict(
                            x=x_[dataset], Model=fit, Data=dataset, y=pred, Error=pred - y_[dataset]
                        )
                    )
                )
    ...
    y["Train_noise"] = y["Train"] + normal(
        loc=0, scale=np.std(y["Train"]) * noise, size=len(y["Train"])
    )
    ...
    for model in colors.keys():
        (
            test_data[(test_data["Model"] == model)].plot.scatter(
    ...
    train_set = X["Train"] + normal(scale=np.std(f(X["Train"]))) * 0.2
    test_set = X["Test"].copy()

    sample_sizes = np.arange(0.1, 1.0, 0.01)
    indices = ([len(train_set), len(test_set)] * sample_sizes.reshape(-1, 1)).astype(int)
    result = []
    lr = LinearRegression()
    ...
    for i, model in enumerate(models.keys()):
        result.loc[model, ["Train RMSE", "Test RMSE"]].plot(
            ax=axes[i], title=f"Model: {model}", logy=True, lw=2
        )
