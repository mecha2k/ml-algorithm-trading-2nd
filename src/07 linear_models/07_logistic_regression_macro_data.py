...
## Data Prep
# To obtain a binary target variable, we compute the 20-quarter rolling average of the annual growth rate of quarterly
# real GDP. We then assign 1 if current growth exceeds the moving average and 0 otherwise. Finally, we shift the
# indicator variables to align next quarter's outcome with the current quarter.
...
# The summary indicates that the model has been trained using maximum likelihood and provides the maximized value of
# the log-likelihood function at -67.9.
model = sm.Logit(data.target, sm.add_constant(data.drop("target", axis=1)))
result = model.fit()
result.summary()

# The LL-Null value of -136.42 is the result of the maximized log-likelihood function when only an intercept is included.
# It forms the basis for the pseudo-R2 statistic and the Log-Likelihood Ratio (LLR) test.
# The pseudo-R2 statistic is a substitute for the familiar R2 available under least squares. It is computed based on
# the ratio of the maximized log-likelihood function for the null model m0 and the full model m1 as follows:
# The values vary from 0 (when the model does not improve the likelihood) to 1 where the model fits perfectly and the
# log-likelihood is maximized at 0. Consequently, higher values indicate a better fit.
...
plt.savefig("images/07-01-logistic_example.png", bbox_inches="tight", dpi=300)
