...
def clean_doc(d):
    ...
    for t in d:
        if not any(
            [t.is_stop, t.is_digit, not t.is_alpha, t.is_punct, t.is_space, t.lemma_ == "-PRON-"]
        ):
            doc.append(t.lemma_)
    return " ".join(doc)
...
def preprocess(articles):
    ...
    clean_articles = []
    for i, doc in enumerate(nlp.pipe(iter_articles, batch_size=100, n_threads=8), 1):
        if i % 1000 == 0:
...
pd.Series(article_length).describe(percentiles=np.arange(0.1, 1.0, 0.1))
